...
int main(int argc, char** argv) {
    ...
    std::string filename = "/home/crispq/label_propagation/ParallelCommunityDetection/GraphExamples/small_test.txt";
    ...
    for(auto local_vtx : (*graph.local_nodes))
    {
        ...
        if(rank == 1)
            std::cout << "I'm vtx : " << local_vtx.id << " with neighbors : " << std::endl;
        ...
        for(auto neigh : (*local_vtx.edges)){
            ...
            if(rank == 1)
                std::cout << n_idx  << " " << std::endl;

            // if(rank == 1)
            //     std::cout << "nº local vtx : " << graph.get_local_vtx() << std::endl;
            // if its local vtx : 
            // if(graph.get_vtx_begin() <= n_idx && n_idx < graph.get_vtx_end()){
            if(n_idx < graph.get_local_vtx()){
                n_label = (*graph.local_nodes)[n_idx].current_label; 
                
                // if(rank == 1)
                //     std::cout << "LOCAL neighbor " << n_idx << " has label " << n_label << std::endl;
            }else{
                int local_idx = graph.get_vtx_end()-n_idx;
                local_idx = graph.from_local_ghost_to_index(n_idx); 
                n_label = (*graph.ghost_nodes)[local_idx].current_label; 

                int original = -1 ; 
                for(auto a : graph.ghost_global_ids)
                    if (a.second == local_idx )
                        original = a.first; 
                // n_label = (*graph.ghost_nodes)[local_idx].current_label; 
                // if(rank == 1)
                //     std::cout << "My neighbor " << n_idx << " indexed " << local_idx << " has label " << n_label << " its ID beign originally " << original << std::endl;
            ...
            // if(rank == 1){
            //     std::cout << "CURRENT label " << n_label << " has value : " << label_cnt[n_label] << " max now is :" << max_label_value << std::endl;
            // }
            ...
            if(max_label_value < label_cnt[n_label])
            {
                ...
                // if(rank == 1)
                //     std::cout << "IF:: LABEL " << n_label << " is now maximal " << max_labels.size() << std::endl;
            ...
            // if new label is the same value another one found previously
                ...
                // if(rank == 1)
                //     std::cout << "ELSE:: Pushing into max_labels with size " << max_labels.size() << std::endl; 
            }
        }
        ...
        if(max_labels.size() >= 1){
            ...
            // if(rank == 1)
            //     std::cout << "Keeping label nº " << rng_label << " -> " << max_labels[rng_label] << std::endl; 
