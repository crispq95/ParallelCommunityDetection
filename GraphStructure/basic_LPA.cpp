...
void run_with_compression_LPA(DistributedGraph *graph, CommunicationHandler *cm, int ns){
    int nsteps = 0; 
    int end_condition = 0;
    ...
    while(end_condition == 0 && nsteps < ns){
        ...
        for(auto local_vtx : (*graph->get_local_vertices())){
            if(local_vtx.is_boundary || !local_vtx.active) // for non boundary vtx and active 
                continue; 
        ...
        for(auto local_vtx : (*graph->get_local_vertices())){
            if(!local_vtx.is_boundary || !local_vtx.active) // for boundary vtx 
                continue; 
